...
class MFEmbedder(Model):
    def __init__(self, init_func, vsize, embdim, symmetric):
        super(MFEmbedder, self).__init__()
        self.V = tf.Variable(init_func(vsize, embdim), name="vectors")
        if not symmetric:
            self.W = tf.Variable(init_func(vsize, embdim), name="covectors")
        self.symmetric = symmetric
    def call(self, x=None):   
        if self.symmetric:
            return tf.matmul(self.V, self.V, transpose_b=True)
        else: 
...
#         obj = tf.reduce_sum(pos) + tf.reduce_sum(neg) - tf.linalg.trace(pos) - tf.linalg.trace(neg)
...
def GMF(Nij_p_np, Nij_n_np, embed_dim, is_symmetric, n_iters, plot):
    ...
    model = MFEmbedder(normal_weights_initialization, graph_size, embed_dim, is_symmetric)
